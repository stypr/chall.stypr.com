	...
	*/
	...
	function install(){
		...
		if(!$query){
			...
			echo json_encode(['type' => 'mysqli-auth', 'reason' => $error]);
			exit;
		}
		// 1-1.check if everything works okay
		$test = mysqli_query($query, "(SELECT a,b,(ROW_NUMBER() OVER (PARTITION BY a ORDER BY b ASC)) AS c FROM (SELECT 1 a ,2 b UNION SELECT 2,3 UNION SELECT 1,1)x)");
		$row = @mysqli_fetch_assoc($test);
		if(!$row){
			echo json_encode(['type' => 'mysqli-update']);
			exit;
		}
		// 1-2. install table
		...
		if(!is_file("./lib/exclude/config.php")){
			echo json_encode(['type' => 'config-error', 'reason' => 'service not installed']);
			exit;
		}
		@unlink("install.php");
		...
		<script>
			...
			function act_install(){
				...
				$.post("?", input, function(d){
					switch(d['type']){
						case "mysqli-auth":
							alert('Login to MySQL server failed. Please auth again.');
							break;
						case "mysqli-update":
							alert('Your MySQL version is outdated. Update to the latest version');
							break;
						case "config-error":
							alert('Your write permissions did not let me install the settings! :( ');
							break;
						case "success":
						location.replace("/");
							break;
